...
说明：工具调用是通过 openai API 的 tool_calls 机制完成的，不是通过模型输出 JSON 文本。当需要调用工具时，系统会自动执行工具并返回结果。你只需要，用自然语言说明你要做什么，**不要**在回答中输出任何 JSON 格式的工具调用信息，等待工具返回结果后，再基于结果回答。无须继续调用工具，直接结束即可。
